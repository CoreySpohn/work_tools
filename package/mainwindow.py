    ...
    def show_deep_work_timing(self):
        if self.deep_work_start:
            ...
            if self.deep_work_counter == 0:
                ...
                Notification(title='Deep work is done', description='You should go do something for a few minutes, good job!', duration=10, urgency='critical').send()
                self.deep_work_start = False
            self.ui.deep_work_label.setText(f'{datetime.timedelta(seconds = self.deep_work_counter)} s')
